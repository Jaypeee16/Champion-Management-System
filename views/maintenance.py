...
class MaintenanceView(ctk.CTkFrame):
    # FIX 1: Added *args and **kwargs to safely absorb any user_info passed by dashboard.py
    ...
    def build_ui(self):
        ...
        # FIX 2: Removed 'takefocus=True' which was crashing the widget
    ...
    def render_issues_tab(self):
        # Placeholder for your Professor's upcoming QA/Issues requirement
        frame = ctk.CTkFrame(self.content_frame, fg_color="white", corner_radius=10)
        frame.grid(row=0, column=0, sticky="nsew")
        ctk.CTkLabel(frame, text="Manage Issues Module", font=("Inter", 20, "bold"), text_color="#1E4528").pack(pady=(30, 10))
        ctk.CTkLabel(frame, text="Coming soon based on Phase 6 Requirements.", font=("Inter", 14), text_color="gray").pack()
    ...
    def execute_backup(self):
        ...
        if not selected_tables:
            messagebox.showwarning("No Tables", "Please select at least one table to back up.", parent=self.winfo_toplevel())
            return
        ...
        try:
            ...
            for tbl in selected_tables:
                try:
                    ...
                    pass # Skips tables that might not exist yet (like projects)
            ...
            messagebox.showerror("Backup Failed", str(e), parent=self.winfo_toplevel())
        finally:
    ...
    def execute_restore(self):
        if not getattr(self, "_restore_file_path", None):
            messagebox.showwarning("No File", "Please browse and select a backup file first.", parent=self.winfo_toplevel())
            return

        if not messagebox.askyesno("Confirm Restore", "This will overwrite existing records for the backed-up tables.\nAre you sure?", parent=self.winfo_toplevel()):
            return
        ...
        try:
            ...
            messagebox.showerror("Restore Failed", str(e), parent=self.winfo_toplevel())
        finally:
            if conn.is_connected(): cursor.close(); conn.close()
